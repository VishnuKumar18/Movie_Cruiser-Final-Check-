...
SELECT mo_id, mo_title, mo_has_teaser, mo_box_office, mo_genre FROM movie WHERE mo_active = 'Yes' AND mo_date_of_launch <= now();
...
SELECT COUNT(*) FROM movie INNER JOIN favorites ON movie.mo_id = favorites.fa_mo_id WHERE favorites.fa_us_id = 2;
